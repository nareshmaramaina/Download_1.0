...
int get_RHMS_response()
{
	...
	ret = get_RHMS_Server_details();

	if( ret < 0 )
	{
		strcpy(RHMS_Server.RTMServerIP,"202.53.78.186");
		strcpy(RHMS_Server.PortNo,"5051");
	}
	...
	if ( (strcmp(serialnum,"ERROR") ) == 0 || strlen(serialnum) == 0 )
	{
		...
		if(ret != 0)
			...
			sprintf(start_buff,"#Health#%s#%s",macid,RHMS_Server.ProjectName);
	}
	...
	else
	{
		if(ret != 0)
			...
			sprintf(start_buff,"#Health#%s#%s",serialnum,RHMS_Server.ProjectName);
	...
	addr.sin_addr.s_addr=inet_addr(RHMS_Server.RTMServerIP); 

	addr.sin_port=htons(atoi(RHMS_Server.PortNo));

	fprintf(stdout,"Connecting ... Server Ip Address = %s\t Port Number = %s\n",RHMS_Server.RTMServerIP,RHMS_Server.PortNo);
	...
	sprintf(cmd,"dos2unix %s",RHMS_Response_file);

	system(cmd);

	Parsing_ACK_from_Server();
